...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto","order":"2"}'>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>教室名称</div>
        <el-input v-model="formSearch.jiaoshimingcheng" placeholder="教室名称" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>教室状态</div>
        <el-select v-model="formSearch.jiaoshizhuangtai" placeholder="请选择教室状态" :clearable="true">
          <el-option v-for="(item, index) in jiaoshizhuangtaiOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
	  ...
	  <el-button v-if="isAuth('jiaoshixinxi','新增')" :style='{"cursor":"pointer","border":"1px solid #db961f","padding":"0px 15px","boxShadow":"1px 1px 3px #f8a412","margin":"0px 10px 0 0","color":"#fff","outline":"none","borderRadius":"4px","background":"#f8a412","width":"auto","fontSize":"14px","lineHeight":"40px","height":"40px"}' type="primary" @click="add('/index/jiaoshixinxiAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.jiaoshimingcheng != '') searchWhere.jiaoshimingcheng = '%' + this.formSearch.jiaoshimingcheng + '%';
        if (this.formSearch.jiaoshizhuangtai != '') searchWhere.jiaoshizhuangtai = this.formSearch.jiaoshizhuangtai;
        this.$http.get('jiaoshixinxi/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/jiaoshixinxiDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
